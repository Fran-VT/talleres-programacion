...
date: '2020-11-26',
author: "Alejandro Acevedo-Aracena, _PhD_^[_FONDECYT Postdoctoral Fellow_, Universidad de Chile, deepen.data@gmail.com]; Manuel Muñoz-González^[_Pregrado_, Universidad de Chile]", output: {html_document: {theme: flatly, highlight: tango, toc: true, toc_float: {collapsed: false, smooth_scroll: true}, df_print: paged, code_folding: hide}, html_notebook: default}}
---
...
Foreach es un comando de C shell, por lo que no lo podremos usar a menos de que 
instalemos csh:
```{bash}
sudo apt install csh
#Abriremos csh utilizando el comando 
csh
```


```{bash}
#Estructura basica del comando foreach
foreach n (lista de elementos)
    comando1 $n  #para los n elementos de la lista
    comando2 $n #para los n elementos de la lista
    end   # termino del loop
```
 
```{bash}
#Generaremos una tabla vacia llamada estructura matriz
#Foreach es un comando de C shell, por lo que no lo podremos usar a menos de que instalemos csh
sudo apt install csh
#abrireremos desde la terminal utilizando 
csh
...
```
```{bash}
#Generar una matriz de las frecuencias de los nucleotidos  para las 9 primeras posiciones
cat >> estructuramatriz
A
C
T
G
#ahora contaremos la frecuencia de los nucleotidos en las primeras 9 posiciones
...
#finalmente agregaremos la frecuencia a la tabla generada al principio

paste estructuramatriz temporal* > Matriz_ACTG.txt
#eliminaremos el archivo temporal que guardaba la frecuencia de los nucleotidos
rm temporal*.txt
```
...
```{bash}
#de manera similar podemos  contar los nucleotidos de todo un archivo lo que podria ser util para estimar el %GC
...
_Word count_ es un comando que permite realizar diferentes conteos, incluyendo palabras y saltos de lineas.
