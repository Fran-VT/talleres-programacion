---
title: "forma 1"
output: html_notebook
---



```{r}
install.packages("phangorn", dep = TRUE)
install.packages("ape")
library(stats)
library(ade4)
library(ape)
library(phangorn)
library(ggplot2)
library(tidyverse)
```

```{r}
myFirstAlignment <- readRDS(file= "Alineamiento_ORF1ab_17_seqs.rds")

label<-myFirstAlignment@unmasked %>% names()
label %<>% str_replace('[A-Z]+.\\d+.\\d.','') %>% str_replace('(c|C)ovid|COVID','') %>% str_replace_all('ORF1ab|Orf1ab','') %>%
            trimws
names(myFirstAlignment@unmasked ) <- label
test <- as.DNAbin(myFirstAlignment)
```


```{r}
png(file=nombre_figura_matriz_distancia, width=600, height=350)
## modelo Tamura&Nei, permite distintas frecuenias de transiciones y transversiones y tasas de sustitucion
#K81,F84, BH87,T92,TN93, GG95, logdet, paralin, indel, indelblock ,<--- estos son los modelos disponibles
D <- dist.dna(test, model = "TN93")
temp <- as.data.frame(as.matrix(D)) 
tabla <-table.paint(temp, cleg = 0, clabel.row = 0.5, clabel.col = 0.5)
dev.off()
```

```{r}
png(file=nombre_figura_arbol_minimum_evolution, width=600, height=350)
tree <- fastme.bal(D)
plot(tree, cex = 0.6)
title("Un arbol utilizando algoritmo de minimum evolution") ### agregar una firma 
dev.off()
```

```{r}
png(file=nombre_figura_arbol_sin_raiz, width=600, height=350)
plot(tree, type = "unrooted", show.tip = FALSE)
title("Unrooted tree")
tiplabels(label, cex = 0.5, fg= "transparent")
dev.off()
```

```{r}
png(file=nombre_figura_arbol_con_raiz, width=600, height=350)
tre2 <- root(tree, out = 1) ### out = 1 es el primer archivo de el archivo de anotaciones.
plot(tre2, show.tip = FALSE, edge.width = 2)
title("Rooted tree")
tiplabels(label, cex = 0.5, fg = "transparent")
axisPhylo()
dev.off()
```








